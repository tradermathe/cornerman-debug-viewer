...
export const Vision3DRule = {
  ...
  mount(_host, state) {
    ...
    host.innerHTML = `
      ...
      <p class="hint">
        ...
        dims any 3D joint whose 2D analogue is low-confidence (the bag-occlusion
        gate from the comparison notebook).
      </p>
  ...
  // No overlay paint on the video canvas — the lens lives entirely in the
  // side panel, beside the video. Returning nothing keeps the video clean.
...
function updateInfo(state, confMask) {
  ...
  // Shoulder-to-wrist extension for both arms, m. Useful sanity check that
  // matches the punch-direction plot from the comparison notebook.
  const ext = side => {
  ...
  infoBox.innerHTML =
    `frame ${f}   ·   L ext ${fmt(ext("L"))} m   ·   R ext ${fmt(ext("R"))} m<br>` +
    (bodyYawDeg !== null
